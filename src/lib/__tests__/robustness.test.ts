...
describe("R1. Crash Resistance", () => {
  ...
  it("R1.40: DESIGN_STYLES length unchanged", () => { expect(DESIGN_STYLES).toHaveLength(13); });
  ...
  it("R1.45: last style is rosepine", () => { expect(DESIGN_STYLES[12].id).toBe("rosepine"); });
...
describe("R2. Correctness — Mathematical Properties", () => {
  ...
  it("R2.21: every style has distinct dark identity", () => {
    ...
    expect(new Set(sigs).size).toBe(13);
  });
